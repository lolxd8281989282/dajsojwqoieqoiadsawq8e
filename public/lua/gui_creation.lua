...
function GUI.Create(Config, ESP, Aimbot)
  ...
  MiscLeftColumn.Parent = MiscPage
  ...
  MiscRightColumn.Parent = MiscPage
...
ConsoleFrame.Parent = OutputPage
...
ButtonContainer.Parent = OutputPage
...
local function addConsoleMessage(message)
  ...
  text.Size = UDim2.new(1, 0, 0, 16)  -- Reduced from 20 to 16
...
PlayerListScrollFrame.Parent = PlayerListPage
...
PlayerInfoFrame.Parent = PlayerListPage
...
-- Add Stream Proof toggle
local streamProofToggle = createToggle("Stream Proof", settingsContainer, UDim2.new(0, 5, 0, currentY), "StreamProof")
...
createConfigButton("Open Folder", UDim2.new(0.66, 5, 0, 5), function()
    ...
    if isfolder("Dracula/Configs") then
        os.execute('explorer.exe "' .. game:GetService("PathService"):ParsePath("Dracula/Configs") .. '"')
    end
end)
...
function updateConfigList()
    ...
    local yPos = 5
    for _, configName in pairs(configSystem.list()) do
